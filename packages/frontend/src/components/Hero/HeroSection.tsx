...
export default function HeroSection() {
  ...
  useEffect(() => {
    ...
    // Create neural network particles - 优化：减少粒子数量
    const particleCount = 1000; // 从 2000 减少到 1000
    ...
    const animate = () => {
      ...
      // Update connections - 优化：减少连接计算频率和范围
      if (Math.random() < 0.1) { // 只在 10% 的帧中更新连接
        const linePositions: number[] = [];
        const maxDistance = 1.2; // 减小连接距离
        const maxConnections = 500; // 限制最大连接数
        let connectionCount = 0;

        // 优化：只检查部分粒子的连接
        const step = Math.max(1, Math.floor(particleCount / 100)); // 只检查 1% 的粒子对
        ...
        for (let i = 0; i < particleCount && connectionCount < maxConnections; i += step) {
          for (let j = i + step; j < particleCount && connectionCount < maxConnections; j += step) {
            ...
            const distanceSq = dx * dx + dy * dy + dz * dz; // 使用平方距离避免 sqrt
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center">
            ...
            <p className="text-lg md:text-xl text-gray-300 mb-10 max-w-3xl mx-auto leading-relaxed">
              {t('hero.subtitle')} <span className="text-purple-400 font-semibold">{t('hero.subtitlePoue')}</span>{t('hero.subtitlePoueDesc')}
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center items-center mb-16">
              ...
              <button 
                ...
                className="px-8 py-4 border border-cyan-500/30 text-cyan-300 rounded-xl text-lg font-semibold hover:bg-cyan-500/10 transition-all neon-glow-cyan flex items-center gap-2"
              >
                {t('hero.afcWallet')}
              </button>
